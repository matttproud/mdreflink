import {describe, it, expect, beforeEach, afterEach, vi, MockInstance}
  from 'vitest';
...
describe('CLI entrypoint', () => {
  ...
  beforeEach(() => {
    ...
    processExitSpy = vi.spyOn(process, 'exit')
...
`;
    ...
    expect(fs.readFile).toHaveBeenCalledWith(filePath, 'utf8');
    expect(writeFileMock).toHaveBeenCalledWith(filePath, expectedOutput,
        'utf8');
    expect(processStdoutWriteSpy).not.toHaveBeenCalled();
  });
  ...
  it('should not call console or process.exit during normal operation',
      async () => {
        const inputMarkdown = `[hello](world)`;
        const filePath = 'test.md';
